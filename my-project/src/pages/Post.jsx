...
const Post = () => {
    ...
    function getDesiredTime(time) {
        ...
        const adjustedHr = hr % 12 || '00';
        return `${adjustedHr}:${time.split(':')[1]} ${amOrPm}`;
    }
    ...
    const postDataFunc = (postData) => {
        setPostFileUrl(ServicesDS.getFile(postData.postImage))
        setPostContent(postData.content)
        setUserId(postData.userId)

        ServicesDS.AllUserAuthengate().then(users => setAllUsers(users.documents))

        authenticationServices.getUser().then(user => setCurrentUserData(user.$id))

        setFileId(postData.postImage)
        console.log(postData);
    }

    useEffect(() => {
        ServicesDS.getPost(postID).then((postInfo) => {
            postDataFunc(postInfo)
            setPostDate(getDesiredTime(postInfo.$createdAt))
    ...
    return (
        <>
            {/* component */}
            <main className="h-full w-full bg-[#262626] flex items-center justify-center">
                <div className="border max-w-screen-md bg-white mt-6 rounded-2xl p-4 w-[65%]">
                    <div className="flex items-center	justify-between">
                        <div className="gap-3.5	flex items-center ">
                            <Link to={`/user/${userId}`}>
                                <img src={facemashUser} className="object-cover bg-yellow-500 rounded-full w-10 h-10" />
                            </Link>
                            <div className="flex flex-col">
                                <Link to={`/user/${userId}`}>
                                    <b className="mb-2 capitalize">{allUsers.map(userData => userData.userId == userId && userData.name)}</b>
                                </Link>
                        ...
                        <div className="bg-gray-100	rounded-full h-3.5 flex	items-center justify-center cursor-pointer ">
                            {userId == currentUserData ? <div className=" flex flex-row justify-center items-center gap-[15px] ">
                                <Link to={`/edit-post/${postID}`}>
                                    <Button value={'Edit Your Post'} />
                                </Link>
                                <Link to={'/'}
                                    onClick={e => (ServicesDS.deletePost(postID), ServicesDS.deleteFile(fileId))} >
                                    <Button value={'Delete Your Post'} />
                                </Link>
                            </div> : <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="34px" fill="#92929D">
                    ...
                    <div className="whitespace-pre-wrap mt-7"> {postContent} </div>
                    <div className="mt-5 flex gap-2	 justify-center border-b pb-4 flex-wrap	">
                        {postFileUrl && <img src={postFileUrl} className="bg-red-500 rounded-2xl w-1/3 object-cover h-96 flex-auto" alt="photo" /> }
                    ...
                    <div className="flex items-center justify-between mt-4">
                        <img src={facemashUser} width={'40px'} />
